...
export default function AdminClient() {
  ...
  return <section className="mx-auto max-w-6xl py-2">
    ...
    {loading ? <div className="flex min-h-48 items-center justify-center text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Loading owner data…</div> : <>
      {tab === "overview" && <div className="space-y-7"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{cards.map(([label, value]) => <div key={String(label)} className="rounded-xl border border-border bg-card p-4 shadow-xs"><p className="text-xs font-semibold text-muted-foreground">{label}</p><p className="mt-2 text-2xl font-bold text-foreground">{value}</p></div>)}</div><div className="rounded-xl border border-border bg-card p-5 shadow-xs"><h2 className="font-bold text-foreground">Language usage</h2>{overview?.language_usage.length ? <ul className="mt-3 space-y-2 text-sm text-foreground">{overview.language_usage.map(item => <li key={item.language} className="flex justify-between border-b border-border pb-2 font-medium"><span>{item.language}</span><span className="font-bold text-primary">{item.count}</span></li>)}</ul> : <p className="mt-2 text-sm text-muted-foreground">No recorded language switches yet.</p>}</div></div>}
      {tab === "content" && <div className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]"><form onSubmit={saveContent} className="rounded-xl border border-border bg-card p-5 space-y-4 shadow-xs"><h2 className="font-bold text-foreground">Edit public copy</h2><p className="text-xs text-muted-foreground">Updates are persisted, publishable, and locale-specific.</p><label className="block text-sm font-semibold text-foreground">Language<select value={form.locale} onChange={e => setForm({ ...form, locale: e.target.value })} className="mt-1 w-full rounded-lg border border-border bg-background p-2 text-foreground font-medium"><option value="en">English</option><option value="hi">Hindi</option></select></label><label className="block text-sm font-semibold text-foreground">Content key<input list="content-keys" value={form.key} onChange={e => setForm({ ...form, key: e.target.value })} className="mt-1 w-full rounded-lg border border-border bg-background p-2 text-foreground font-medium" required /><datalist id="content-keys">{contentSuggestions.map(key => <option key={key} value={key} />)}</datalist></label><label className="block text-sm font-semibold text-foreground">Text<textarea value={form.value} onChange={e => setForm({ ...form, value: e.target.value })} className="mt-1 min-h-32 w-full rounded-lg border border-border bg-background p-2 text-foreground font-medium" required /></label><label className="flex items-center gap-2 text-sm font-semibold text-foreground"><input type="checkbox" checked={form.is_published} onChange={e => setForm({ ...form, is_published: e.target.checked })} />Published</label><button className="btn-primary">Save public content</button></form><div className="rounded-xl border border-border bg-card p-5 shadow-xs"><h2 className="font-bold text-foreground">Saved overrides</h2>{content.length ? <div className="mt-3 space-y-2">{content.map(entry => <button key={entry.id} onClick={() => { setForm({ locale: entry.locale, key: entry.key, value: entry.value, is_published: entry.is_published }) }} className="block w-full rounded-lg border border-border p-3 text-left hover:bg-muted text-foreground transition-colors"><div className="flex justify-between gap-3 text-sm font-semibold"><span>{entry.locale} · {entry.key}</span><span className="text-xs text-muted-foreground font-medium">{entry.is_published ? "Published" : "Draft"}</span></div><p className="mt-1 truncate text-xs text-muted-foreground">{entry.value}</p></button>)}</div> : <p className="mt-3 text-sm text-muted-foreground">No overrides saved yet. Base translations remain in use.</p>}</div></div>}
      {tab === "media" && <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]"><form onSubmit={upload} className="rounded-xl border border-border bg-card p-5 space-y-4 shadow-xs"><h2 className="font-bold text-foreground">Upload image</h2><p className="text-xs text-muted-foreground">JPEG, PNG, WebP, or GIF up to 5 MB. Uploaded files are stored in the application database.</p><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(e: ChangeEvent<HTMLInputElement>) => setFile(e.target.files?.[0] || null)} className="text-sm text-foreground" required /><button className="btn-primary" disabled={!file}>Upload</button></form><div className="rounded-xl border border-border bg-card p-5 shadow-xs"><h2 className="font-bold text-foreground">Uploaded media</h2>{media.length ? <div className="mt-3 space-y-2">{media.map(item => <div key={item.id} className="flex items-center justify-between gap-3 rounded-lg border border-border p-3 text-sm"><a href={`/api/media/${item.id}`} target="_blank" className="truncate text-primary font-semibold hover:underline">{item.filename}</a><span className="text-xs text-muted-foreground font-medium">{Math.ceil(item.size_bytes / 1024)} KB</span><button onClick={() => removeMedia(item.id)} className="text-xs text-destructive font-semibold hover:underline">Delete</button></div>)}</div> : <p className="mt-3 text-sm text-muted-foreground">No uploaded images.</p>}</div></div>}
...
function Table({ headings, rows }: { headings: string[]; rows: string[][] }) {
  return <div className="overflow-x-auto rounded-xl border border-border bg-card shadow-xs"><table className="w-full text-left text-sm text-foreground"><thead className="border-b border-border text-xs text-muted-foreground bg-muted/40"><tr>{headings.map(heading => <th key={heading} className="px-4 py-3 font-semibold">{heading}</th>)}</tr></thead><tbody>{rows.length ? rows.map((row, index) => <tr key={index} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">{row.map((value, cell) => <td key={cell} className="max-w-xs truncate px-4 py-3 font-medium text-foreground">{value}</td>)}</tr>) : <tr><td colSpan={headings.length} className="px-4 py-8 text-center text-muted-foreground">No records yet.</td></tr>}</tbody></table></div>
}
